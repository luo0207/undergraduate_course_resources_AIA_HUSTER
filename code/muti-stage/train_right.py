...
images, sign = data.data_input("data_LBP_differ/train_you.mat")  # 252pos  188neg
# pos_images, neg_images = images[:252], images[252:]
# random.shuffle(pos_images), random.shuffle(neg_images)
# images = np.vstack([pos_images, neg_images])  # 252pos 188neg
# train_images, train_sign = np.vstack([images[0:210], images[252:402]]), np.vstack([sign[0:210], sign[252:402]])  # train 210+150=360
# valid_images, valid_sign = np.vstack([images[210:252], images[402:]]), np.vstack([sign[210:252], sign[402:]])  # valid=80
...
#损失函数
logits, cross_entropy = Loss_ASoftmax(x = h_state, y=y, l=0.0, num_cls=2, m=4 )
...
for j in range(60000):#472
    ...
    if((j+1)%100 ==0):
        saver.save(sess, 'model_LBP_differ/you/10lstm.ckpt_Asoftmax-', global_step=j + 1)
